    ...
    function addHandleMessage(handle) {
        ...
        const atIndex = userText.indexOf("@") >= 0; 

        // find @ position and remove text from @ to first space (that is a user name) and keep the rest of the text. 
        if (atIndex >= 0) {
            const spaceIndex = userText.indexOf(" ", atIndex);

            if (spaceIndex >= 0) {
                userText = userText.substring(spaceIndex);
            }
        }
    ...
    function constantTimeNameFilter(userText) {
        ...
        // Object.keys(userNames).forEach((key)=> console.log(`key: ${key}, value: ${userNames[key].firstName}`));
    }
